    ...
    if callback_data == "order":
        ...
        if API_DATA.get('url') and API_DATA.get('key'):
            ...
            if 'balance' in result:
                ...
                await query.edit_message_text(f"❌ Xato: {{result.get('error', 'Noma\'lum')}}")
        else:
...
async def submit_order(
    ...
    if result and "order" in result:
        ...
        await reply_func(f"❌ Xato: {result.get('error', 'Noma\\'lum')}")
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if callback_data == "clone_list":
        ...
        for idx, (clone_id, clone_data) in enumerate(clones_list[start_idx:end_idx], start=start_idx + 1):
            ...
            username = clone_data.get("username", "Noma'lum")
    ...
    if callback_data.startswith("clone_info_"):
        ...
        text = (
            ...
            f"🆔 ID: {clone_id}\n"
            f"📊 Status: {status}\n"
            f"👤 Username: @{clone_data.get('username', 'Noma\'lum')}\n"
            ...
            f"📅 Yaratilgan: {clone_data.get('created_at', 'Noma\'lum')}\n"
        )
